...
from .charge3net.models.densitymodel import  PaiNNQMEncoder
...
class Trainer:
    ...
    def _train_epoch(self):
        for batch in self.train_dl:
            ...
            # 2. 然后对计算出的梯度进行裁剪，同时用一个变量接收算出来的全局梯度范数
            ...
            if self.local_rank == 0 and self.global_rank == 0:
                ...
                if self.step % self.log_steps == 0:
                    ...
                    logging.info(f"step: {self.step} train/loss: {loss.item():.6f} grad_norm: {total_norm.item():.4f} lr: {last_lr:.6f}")
                    
                    # 打印刚才保存下来的 total_norm 到 TensorBoard
                    self.tensorboard.add_scalar("train/grad_norm", total_norm, global_step=self.step)
    ...
    def _is_painn_qm_encoder(self) -> bool:
        """判断当前模型是否为新的 PaiNNQMEncoder（DDP 包装后取 .module）。"""
        m = self.model.module if hasattr(self.model, "module") else self.model
        return isinstance(m, PaiNNQMEncoder)
            ...
            if batch["num_probes"] > max_predict_batch_probes:
                ...
                for i, sub_batch in enumerate(pred_utils.split_batch(batch, max_predict_batch_probes)):
                    ...
                    if atom_repr is None:
                        # PaiNNQMEncoder: atom_model 返回 (S_JK, V_JK) tuple
                        # E3nn QMEnvironmentEncoder: atom_model 返回 node_list
                        atom_repr = self.model.module.atom_model(sub_batch)

                    if self._is_painn_qm_encoder():
                        # 新架构：probe_model(input_dict, S_JK, V_JK)
                        S_JK, V_JK = atom_repr
                        outputs = self.model.module.probe_model(sub_batch, S_JK, V_JK)
                    else:
                        # E3nn：probe_model(input_dict, node_list)
