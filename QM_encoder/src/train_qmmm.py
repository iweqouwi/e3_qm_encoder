...
def build_model(cfg: dict, cutoff: float) -> torch.nn.Module:
    ...
    if backend == "painn":
        ...
        # 初始化读出层最后一层 bias → log(ρ_mean)，帮助网络在训练初期快速定位密度量级
        bias_init = p.get("output_bias_init", -11.5)
        _init_output_bias(model, bias_init)
        ...
        # E3nn 同样需要偏置初始化（偏置值与 PaiNN 相同）
        _init_output_bias(model, -11.5)
...
def _init_output_bias(model: torch.nn.Module, bias_value: float) -> None:
    ...
    物理动机：模型输出的是对数密度 log(ρ)，电子密度的典型均值约为
    ρ_mean ≈ 1e-5 a.u.，对应 log(ρ_mean) ≈ -11.5。
